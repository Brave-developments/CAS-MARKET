Citizen.CreateThread(function()
    ...
    if CAS.Ped == nil then
        ...
        SetPedDefaultComponentVariation(CAS.Ped)
        SetPedRandomProps(CAS.Ped)
        SetPedRandomComponentVariation(CAS.Ped, true)
        TaskStartScenarioInPlace(CAS.Ped, "WORLD_HUMAN_SMOKING", 0, true)
        FreezeEntityPosition(CAS.Ped, true)
        SetEntityInvincible(CAS.Ped, true)
        SetBlockingOfNonTemporaryEvents(CAS.Ped, true)
        SetPedCanRagdoll(CAS.Ped, false)
        SetPedFleeAttributes(CAS.Ped, 0, false)
        SetPedCombatAttributes(CAS.Ped, 46, true)
        SetPedConfigFlag(CAS.Ped, 17, true)
        AddRelationshipGroup("CAS_PedGroup")
        SetPedRelationshipGroupHash(CAS.Ped, GetHashKey("CAS_PedGroup"))
        SetRelationshipBetweenGroups(0, GetHashKey("CAS_PedGroup"), GetHashKey("PLAYER"))
        SetRelationshipBetweenGroups(0, GetHashKey("PLAYER"), GetHashKey("CAS_PedGroup"))
    end
